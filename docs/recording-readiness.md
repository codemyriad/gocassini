...
optional transcription separately. Existing recordings remain available when
the recording connection needs attention.
...
- `GET /operator/health`: media-host, recording and optional transcription
  checks, using cached host and network findings. `recording_state` covers
  recording and playback; aggregate `state` also includes optional processing.
