...
The `asrregression` build tag runs a [two-track synthetic meeting regression](../cassini-go-recorder/internal/transcribe/testdata/synthetic-boundary/README.md) with no private audio or meeting text. The identical test fails on the actual pre-PR source and passes on this PR on CPU and CUDA, checking the missing spoken phrase rather than configuration or word counts. The fixture, generator, provenance and reproduction commands are included. A second synthetic acknowledgement exposed an empty-output regression in the initial PR. Empty v3 VAD decodes now receive one retry with surrounding recorded audio, bounded by the detector’s existing silence interval, using the same decoder and hints. Successful initial decodes are unchanged. The acknowledgement test also covers recording edges, interior speech, repeated turns and silence, with strict word-count and timestamp assertions. Both tests run in ordinary PR CI on the hosted CPU runner, using cached, checksum-verified FP32 Parakeet v3 and Silero models. There is no GPU or `full-ci` requirement and no allowed failure. The build tag only makes them optional for fast local unit runs; selecting it without the models fails rather than skipping.
