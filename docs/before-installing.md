...
| A compatible execution host | Published Cassini images support Linux amd64 and arm64. Check the host where the ExApp will execute, which can differ from Nextcloud's host. CPU transcription is supported; a GPU is optional | Execution-host administrator |
...
The CPU image runs natively on Linux amd64 and arm64 execution hosts, including
64-bit ARM machines. The optional NVIDIA/CUDA image requires x86_64. The execution
host can be separate from Nextcloud; emulation is not required for supported architectures.

Once eligible, follow [the installation guide](exapp-install.md), then use Cassini
Operator → Publish pipeline to check credentials, storage, processing, and a real Talk recording. The
...
> AppAPI with a working ExApp deployment engine, a Linux amd64 or arm64 execution host,
