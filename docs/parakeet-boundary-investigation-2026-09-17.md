...
Additional blind audio checks have confirmed an omitted passage recovered by the new path, genuine repeated greetings, and removal of a repetition present in the published transcript. A Gemini response hallucinated speech over a near-silent clip (about −92 dBFS RMS); that response is excluded from scoring. A further audit of four large word-count changes confirmed one removed repetition and one recovered passage, but also two genuine omissions: one 30-second window lost all speech, and another retained only part of the passage. The missing speech was absent from all participant outputs in those windows. Full-track PCM hashes matched the validated replay exactly. These selected extremes do not estimate an overall error rate; they establish that successful archive execution is insufficient to approve the recognition change. Diagnosis is ongoing. Published transcripts and model references are audit aids, not ground truth.
