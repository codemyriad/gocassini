...
The final normal individual-track beam replay completed all **138 meetings and 678 participant tracks on the GPU**, with no processing failures or invalid word timestamps. Every decoded PCM hash and sample count matched the independently audited extraction. Validation requires the exact source/track inventory, result and configuration hashes, and effective per-track hint files; earlier greedy results do not count toward completion. Supported hints were active on 670 tracks, with empty effective vocabularies verified separately. These are execution, timing and input-integrity checks, not a claim of perfect recognition; blind audio checks of selected large transcript differences found both recoveries and substantial new omissions, described below. Those omissions remain a rollout blocker. No meeting summaries were generated. Source integrity is checked separately from decoded duration: timestamp-preserving silence can hide missing packets in a damaged recording. Twenty truncated historical MKVs were reconstructed from retained packet archives. All 5,517,760 audio packets across their 127 tracks were verified before replay; the original recordings were left untouched.
...
Additional blind audio checks have confirmed an omitted passage recovered by the new path, genuine repeated greetings, and removal of a repetition present in the published transcript. A Gemini response hallucinated speech over a near-silent clip (about −92 dBFS RMS); that response is excluded from scoring. A further audit of four large word-count changes confirmed one removed repetition and one recovered passage, but also two genuine omissions: one 30-second window lost all speech, and another retained only part of the passage. The missing speech was absent from all participant outputs in those windows. Full-track PCM hashes matched the validated replay exactly. These selected extremes do not estimate an overall error rate; they establish that successful archive execution is insufficient to approve the recognition change. Sixteen controlled GPU replays held full-track PCM, frontend and VAD input fixed. Whole-span greedy reduced reference disagreements from 65 to 4 and from 57 to 11 in the two affected windows. Hinted beam with the old boundary policy reduced them to 11 and 13. Changing only subdivision while retaining 30 ms of recorded context and no synthetic tail produced 10 and 40 disagreements. VAD traces cover the omitted speech, including a short 3.3-second span; neither a universal ten-second limit nor retrying only empty output addresses every failure. A factorial comparison of recorded context and synthetic tail showed opposing effects across the two passages: padding helped one and worsened the other. Removing hints from whole-span beam produced byte-identical word results on both full tracks, so disabling hints alone cannot fix these omissions. These tests establish decoder/boundary sensitivity, not the specific upstream scoring mechanism. A replacement policy must preserve the original recoveries, useful hints and false-speech controls; no general remedy is validated yet. Published transcripts and model references are audit aids, not ground truth.
...
- Added a resumable full-meeting GPU replay that processes every participant track through the public production recognizer. The final beam-policy replay completed all 138 meetings and 678 tracks; earlier greedy results were not reused to satisfy this requirement.
