...
### Portable `.opus`
The one canonical, user-facing Cassini meeting format and the only durable, published contract. A single file containing playable Opus audio plus the embedded `org.cassini.portable-meeting/1` manifest, published at <https://format.gocassini.com/>.
