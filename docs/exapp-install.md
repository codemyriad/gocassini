...
| `OPENROUTER_API_KEY` | No | Remote meeting-summary API key. Remote mode sends transcript text to that endpoint. Unset, summaries are skipped unless local mode is enabled |
| `CASSINI_SUMMARY_BACKEND` | No | `local` enables the [bundled Ling pilot](./local-summaries.md) without an API key; default `remote` preserves existing behavior. `off` skips summaries |
