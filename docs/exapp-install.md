...
up somewhere nobody is looking — or somewhere everybody can read.
...
leftover copy at the other root. See [Switching storage
...
| `/operator/setup` | USER | Whether recordings can be served at all, why not in one plain sentence, which storage model is in force, and two AI capability bits — `{"ok":…,"state":…,"mode":…,"cause":…,"features":{"summaries":…,"insights":…}}` and nothing else. `cause` is empty whenever recordings can be served, and whenever the honest reason would name an account, a path or an app. `awaiting_choice` is still in the shape and is always `false`: nothing asks an administrator to choose a storage model any more (D-753/D-756), and it is kept only so a client that has not been rebuilt still parses this response |
