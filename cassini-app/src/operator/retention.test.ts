...
describe("retention control", () => {
  it("copies the group on first split and restores inactive policies on later toggles", () => {
    const group: RetentionGroup = { mode: "group", policy: { forever: false, count: 14, unit: "days" }, fine: { failed_capture: { forever: true }, failed_build: { forever: true } } };
