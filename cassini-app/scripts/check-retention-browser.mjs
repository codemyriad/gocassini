...
let saved = { version: 2, revision: 0, recordings: forever(), history: group(["failed_capture", "failed_build", "superseded", "failed_publish"]), current: forever(), logs: forever() };
let puts = 0;
...
try {
  ...
  await recordings.getByLabel("Source recordings count", { exact: true }).fill("2");
  await recordings.getByLabel("Source recordings unit", { exact: true }).selectOption("weeks");
  ...
  await history.getByLabel("Whole group count", { exact: true }).fill("3");
  await history.getByRole("combobox", { name: "Policy control" }).selectOption("fine");
  assert.equal(await history.getByLabel("Failed recordings count").inputValue(), "3");
  await history.getByLabel("Failed recordings count").fill("1");
  await history.getByRole("combobox", { name: "Policy control" }).selectOption("group");
  await history.getByRole("combobox", { name: "Policy control" }).selectOption("fine");
  assert.equal(await history.getByLabel("Failed recordings count").inputValue(), "1");
  assert.equal(puts, 0, "editing must not write settings");
  await page.getByRole("button", { name: "Save retention settings" }).click();
  await page.getByRole("status").waitFor();
  await page.reload();
  assert.equal(await recordings.getByLabel("Source recordings count").inputValue(), "2");
  assert.equal(await recordings.getByLabel("Source recordings unit").inputValue(), "weeks");
  saved.revision++; // another administrator saves first
  await recordings.getByLabel("Source recordings count").fill("4");
  await page.getByRole("button", { name: "Save retention settings" }).click();
  ...
  await recordings.getByLabel("Source recordings count").fill("0");
  const previousPuts = puts;
  await page.getByRole("button", { name: "Save retention settings" }).click();
  assert.equal(puts, previousPuts, "invalid count must not submit");
  await recordings.getByLabel("Source recordings count").fill("1");
  await page.getByRole("button", { name: "Save retention settings" }).click();
  await page.getByRole("status").waitFor();
