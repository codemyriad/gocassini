...
Search knows the names transcription gets wrong. Looking for "cassini" also finds the recordings where it was written down as "casino", and each result says whether it matched your words or a known mis-hearing of them. Administrators keep that list in the app's settings, one name per line with its spellings after it, and Cassini ships a starting list. The participant and project vocabulary beside it now guides transcription itself, so the recogniser can prefer a name it used to invent a spelling for.
