...
func TestOpenStoreEnsuresSchemaAndEmptyList(t *testing.T) {
	...
	if versions := migrationVersions(t, store.db); len(versions) != 9 || versions[0] != 1 || versions[1] != 2 || versions[2] != 3 || versions[3] != 4 || versions[4] != 5 || versions[5] != 6 || versions[6] != 7 || versions[7] != 8 || versions[8] != 9 {
		t.Fatalf("expected migration versions [1 2 3 4 5 6 7 8 9], got %v", versions)
	}
