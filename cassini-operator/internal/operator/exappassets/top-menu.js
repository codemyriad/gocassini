 ...
 * (entry names double as URL prefixes: control-panel/); we fill #content with
 * an iframe pointing there. The embedded page's CSP allows same-host frames,
 * and the iframe shares the user's Nextcloud session, so the proxy's per-route
 * access levels keep applying inside the frame.
 */
(function () {
	"use strict";
