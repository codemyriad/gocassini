...
`)
	...
	opusPath, err := packAttemptMeetingToOpus(context.Background(), bin, workRoot, jobID, 1, "", nil)
	if err != nil {
...
func TestPackAttemptMeetingToOpusKeepsAttemptsApart(t *testing.T) {
	...
	first, err := packAttemptMeetingToOpus(context.Background(), bin, workRoot, jobID, 1, "", nil)
	if err != nil {
		t.Fatalf("seal attempt 1: %v", err)
	}
	second, err := packAttemptMeetingToOpus(context.Background(), bin, workRoot, jobID, 2, "", nil)
	if err != nil {
...
`)
	...
	if _, err := packAttemptMeetingToOpus(context.Background(), bin, workRoot, jobID, 1, "Daily Meeting", nil); err != nil {
		t.Fatalf("packAttemptMeetingToOpus() error = %v", err)
	}
}
...
`)
	...
	if _, err := packAttemptMeetingToOpus(context.Background(), bin, workRoot, jobID, 1, "   ", nil); err != nil {
		t.Fatalf("packAttemptMeetingToOpus() error = %v", err)
	}
}
...
func TestPackAttemptMeetingToOpusErrorsWhenMeetingMissing(t *testing.T) {
	...
	if _, err := packAttemptMeetingToOpus(context.Background(), bin, workRoot, "missing", 1, "", nil); err == nil {
...
func TestPackAttemptMeetingToOpusErrorsWhenOutputMissing(t *testing.T) {
	...
	bin := writeFakeCassini(t, "exit 0\n")
	_, err := packAttemptMeetingToOpus(context.Background(), bin, workRoot, jobID, 1, "", nil)
