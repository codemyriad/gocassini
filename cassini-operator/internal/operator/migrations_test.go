...
func TestMigrationsRunContiguouslyThroughCaptureReceipts(t *testing.T) {
	migrations, err := loadMigrations()
	if err != nil {
		t.Fatalf("loadMigrations() error = %v", err)
	}
	last := migrations[len(migrations)-1]
	if last.Version != 12 || last.Name != "capture_receipts" {
		t.Fatalf("last migration = %04d_%s, want 0012_capture_receipts", last.Version, last.Name)
	}
}
