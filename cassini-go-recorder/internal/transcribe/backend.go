...
type RecognizerFactory func(paths ModelPaths, vadModelPath, provider string, numThreads int, decoder *DecoderConfig) (SpeechRecognizer, error)
...
var (
	...
	backendRegistry = map[string]RecognizerFactory{
		SherpaOnnxBackend: func(paths ModelPaths, vadModelPath, provider string, numThreads int, decoder *DecoderConfig) (SpeechRecognizer, error) {
			return NewRecognizer(paths, vadModelPath, provider, numThreads, decoder)
		},
	}
)
...
// fallback.
func NewRecognizerForBackend(id string, paths ModelPaths, vadModelPath, provider string, numThreads int, decoder *DecoderConfig) (SpeechRecognizer, error) {
	id = ResolveRecognizerBackend(id)
	backendMu.RLock()
	...
	rec, err := factory(paths, vadModelPath, provider, numThreads, decoder)
	if err != nil {
		return nil, err
	}
	if rec == nil {
