...
func TestResolveStreamConcurrency(t *testing.T) {
	...
	if got := resolveStreamConcurrency(1, 8, "cpu"); got != 1 {
		t.Errorf("single stream must be sequential, got %d", got)
	}
	if got := resolveStreamConcurrency(4, 8, "cpu"); got != 4 {
		t.Errorf("4 streams / 8 threads -> 4, got %d", got)
	}
	if got := resolveStreamConcurrency(8, 4, "cpu"); got != 4 {
		t.Errorf("bounded by thread budget 4, got %d", got)
	}

	// GPU stays serial regardless of streams/threads/RAM: concurrent CUDA
	// recognizer creation is unsafe / VRAM-bound (GPU Talk e2e regression).
	if got := resolveStreamConcurrency(4, 8, "cuda"); got != 1 {
		t.Errorf("cuda must be sequential, got %d", got)
	}
	if got := resolveStreamConcurrency(8, 8, "CUDA"); got != 1 {
		t.Errorf("cuda (any case) must be sequential, got %d", got)
	}

	// Memory bound: 3600MB / 1800MB-per-model = 2 (CPU path).
	availableMemMB = func() int { return 3600 }
	if got := resolveStreamConcurrency(4, 8, "cpu"); got != 2 {
		t.Errorf("memory should cap concurrency to 2, got %d", got)
	}
	// Critically low RAM -> sequential.
	availableMemMB = func() int { return 500 }
	if got := resolveStreamConcurrency(4, 8, "cpu"); got != 1 {
	...
	if got := resolveStreamConcurrency(8, 8, "cpu"); got != 3 {
		t.Errorf("override 3, got %d", got)
	}
	if got := resolveStreamConcurrency(2, 8, "cpu"); got != 2 {
		t.Errorf("override capped by streams=2, got %d", got)
	}
	// Explicit override is the GPU escape hatch: it wins over the serial default.
	if got := resolveStreamConcurrency(8, 8, "cuda"); got != 3 {
		t.Errorf("explicit override should win on cuda, got %d", got)
	}
}
