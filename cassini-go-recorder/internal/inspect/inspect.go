...
func InspectPath(out io.Writer, path string) error {
	...
	if meetingPath, ok := detectMeetingMKVPath(path); ok {
		return inspectMeetingMKV(out, meetingPath)
	}
	if portablePath, ok := detectPortableAudioPath(path); ok {
		return inspectPortableAudio(out, portablePath)
	}
	...
	return inspectArchive(out, path, header, records)
}

func inspectArchive(out io.Writer, source string, header cassette.Header, records []cassette.Record) error {
	...
	if len(records) == 0 {
		return errors.New("archive contains zero records")
	}
	return nil
}

func inspectSessionArtifact(out io.Writer, sessionPath string) error {
	...
	fmt.Fprintf(out, "session=%s room=%s started=%s streams=%d logical_tracks=%d participants=%d\n",
		sess.SessionID,
		sess.Platform.Room,
		sess.StartedWallUTC,
	...
	fmt.Fprintf(out, "session_json=%s events=%s events_bytes=%d events_lines=%d streams_dir=%s\n",
		sessionJSON,
		eventsPath,
		eventsSize,
		eventCount,
	...
	for _, stream := range sess.PacketStreams {
		...
		if err != nil {
			fmt.Fprintf(out, "stream=%s error=%v\n", stream.StreamID, err)
			continue
		}
		...
		if summary.srClockRateEstimate > 0 {
			fmt.Fprintf(out, "  sr_clock_rate_est=%.2f\n", summary.srClockRateEstimate)
		}
		...
		if validationErr != nil {
			fmt.Fprintf(out, "  validation_error=%v\n", validationErr)
		...
		if idxInfo, err := os.Stat(path + ".idx"); err == nil {
			fmt.Fprintf(out, "  index=%s bytes=%d\n", path+".idx", idxInfo.Size())
		}
	}

	if len(inspected) > 0 {
		fmt.Fprintln(out, "segment_churn:")
		printSegmentChurn(out, inspected, logicalByLTID)
	}

	if reasons, err := streamCloseReasons(eventsPath); err == nil && len(reasons) > 0 {
		fmt.Fprintln(out, "stream_close_reasons:")
		...
		for _, reason := range keys {
			fmt.Fprintf(out, "  reason=%s count=%d\n", reason, reasons[reason])
		}
	}
	return nil
}

func inspectMeetingMKV(out io.Writer, path string) error {
...
func printSegmentChurn(out io.Writer, streams []inspectedSessionStream, logicalByLTID map[string]session.LogicalTrack) {
...
func metadataTag(tags map[string]string, keys ...string) string {
	for _, key := range keys {
		...
		for tagKey, value := range tags {
			if strings.EqualFold(tagKey, key) && strings.TrimSpace(value) != "" {
				return value
			}
		}
	}
