...
#   4. wire Talk recording: feed Cassini AIO's signaling INTERNAL_SECRET and hand
#      spreed's recording backend to Cassini
...
# Persistent state (HaRP shared key, recording secret) outside the repo. A fixed
...
register_cassini() {
  ...
  local rec_file="$STATE_DIR/recording_secret"
  if [[ ! -s "$rec_file" ]]; then openssl rand -hex 32 > "$rec_file"; chmod 660 "$rec_file"; fi
  local rec_secret; rec_secret="$(cat "$rec_file")"
  ...
  occ app_api:app:register "$CASSINI_APPSTORE_ID" "$HARP_DAEMON" \
    ...
    --env "CASSINI_TALK_RECORDING_SECRET=$rec_secret" \
...
handoff_talk_recording() {
  ...
  local rec_secret; rec_secret="$(cat "$STATE_DIR/recording_secret")"
...
Cassini wired onto AIO.
  ...
  State dir:  $STATE_DIR  (HaRP key, recording secret)
EOF
