# Cassini Venture Brief & Product Positioning
...
# 2.0 Proposal - the file format as center, with UI to consume it

The use case for transcriptions plus audio that can be seeked by word clicking has a myriad of uses, but there's currently no standard to embed data in a way that UIs can offer the transcription and the audio together. There's a lot of experimentation that can be done in the area, like showing what's being said one big full screen word at a time.

The initial strategy of cassini is to use the practical meeting recording and archiving as a way to experiment with file format and UI with real data. Before settling on a format we need to experiment with clear goals for the format in mind. Dogfooding the meeting data should enable a better understanding of the problem.
