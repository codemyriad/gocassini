...
| `IMAGE_REF=... ./harness/bin/ci-e2e-browser-call-capture.sh` | `local-http` + `full` + `cassini installed-exapp` + `recording installed-exapp` | Missing source-capture seam against the exact image: two real Chromium participants join the real Talk/HPB call and send audio; opted-in Alice switches microphone to produce multiple segments and uploads through AppAPI onto the ExApp's disk, while same-call no-consent Bob stores nothing. |
